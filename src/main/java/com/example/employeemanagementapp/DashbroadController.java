...
//        );
    ...
    private void setupDepartmentChoice() {
        ...
        employees.stream().map(Employee::getEmail).distinct().forEach(depts::add);
        ...
        empCol.setCellFactory(col -> new TableCell<>() {
            ...
            protected void updateItem(Employee item, boolean empty) {
                ...
                if (empty || item == null) {
                    ...
                    iv.setFitHeight(32);
                    try {
                        iv.setImage(new Image("", true));
                    } catch (Exception ex) {
                        // fallback
                    }
                    Label name = new Label(item.getFirst_name());
...
//                .filter(e -> dept.equals("All Departments") || e.getDepartment_id().equals(dept))
...
//        updateTopPerformers();
    ...
    private void updateStats() {
        ...
        long checkedIn = employees.stream().filter(e -> "checked-in".equals(1)).count();
    ...
    private void updateTopPerformers() {
        List<Employee> top = employees.stream()
                .sorted(Comparator.comparingInt(Employee::getEmployee_id).reversed())
                .limit(10)
                .collect(Collectors.toList());
